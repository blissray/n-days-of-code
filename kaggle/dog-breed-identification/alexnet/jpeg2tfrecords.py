...
flags.DEFINE_integer("image_height", "224", "The boolean vairable of dog faces cropping [128]")
flags.DEFINE_integer("image_width", "224", "The boolean vairable of dog faces cropping [128]")
...
def persistence_image_data_to_tfrecords(x_data, y_data, data_type,
    ...
    teamlab_util.check_directory("./resized_image")
    teamlab_util.check_directory("./tfrecords")
    teamlab_util.check_directory("./tfrecords/cropping")
    teamlab_util.check_directory("./tfrecords/cropping/train")
    teamlab_util.check_directory("./tfrecords/cropping/test")
    teamlab_util.check_directory(OUTPUT_DIR)
    ...
    for images_filename,breed in zip(x_data,y_data):
        ...
        for image in image_list:
            try:
                imsave(
                    "./resized_image/" +images_filename+"_"+ str(current_index) +".jpeg",
                    sess.run(image))
            except OSError as e:
                print(e)
            ...
            resized_image = tf.image.resize_images(
                image, [FLAGS.image_width, FLAGS.image_height])
